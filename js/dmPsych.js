const dmPsych = (function() {
  ...
  */
  ...
  // create tile game
  obj.MakeTileGame = function(hex, tileHit, tileMiss, roundLength, gameType, nTrials, pM, blockName, roundNum, nOddBall) {

    let losses = 0, round = 1, streak = 0, trialNumber = 0, tooSlow = null, tooFast = null, totalTokens = 0, message;

    let oddBallValue_bern = Math.round(10 * pM);

    let oddBallValue_geom = Math.round(10 * (pM / (1 - pM)));
    ...
    const oddballFeedback = (gameType == "strk") ? "{strk-feedback}" : (blockName == "practice") ? "Miss!" : `+${oddBallValue_bern} Tokens`;
    ...
    const oddball_html = `<div class="outcome-container">
                          <div class="header-oddball">{header}</div>
                          <div class="oddball-text">${oddballFeedback}</div>
                        </div>`;
    ...
    const makeFeedbackArray = function(nOddBall) {
      let nNormal = 10 - nOddBall;
      let array = Array(10-nOddBall).fill("normal").concat(Array(nOddBall).fill("oddball"));
      return jsPsych.randomization.repeat(array, 1);
    };

    const latency = dmPsych.makeRT(nTrials, pM, roundLength, gameType);

    let winArray = makeFeedbackArray(nOddBall);
    let lossArray = makeFeedbackArray(nOddBall);
      ...
      stimulus: () => {
        if (gameType == 'bern') {
          if (tooSlow) {
            ...
            message = (feedbackType == "normal" && blockName !== "practice") ? noTokens_html : (feedbackType == "oddball" && blockName !== "practice") ? oddball_html : noTokens_html;
            if (blockName !== "practice") {
              (feedbackType == "oddball") ? totalTokens += oddBallValue_bern : totalTokens += 0;
            };            
            round++;
            if (lossArray.length == 0) {
              lossArray = makeFeedbackArray(nOddBall);
            };         
          } else {
            let feedbackType = winArray.pop();
            message = (feedbackType == "normal" && blockName !== "practice") ? tokens_html : (feedbackType == "oddball" && blockName !== "practice") ? oddball_html : tokens_html;
            if (blockName !== "practice") {
              (feedbackType == "oddball") ? totalTokens += oddBallValue_bern : totalTokens += 10;
            };               
            round++;
            if (winArray.length == 0) {
              winArray = makeFeedbackArray(nOddBall);
            };             
          };
        ...
        if (gameType == 'strk') {
          if (tooSlow && streak > 0) {
            ...
            let feedbackType = lossArray.pop();
            let nTokens = (feedbackType == "oddball" && blockName !== "practice") ? oddBallValue_geom : finalStreak*10;
            streak = 0;
            message = (feedbackType == "oddball" && blockName !== "practice") ? oddball_html : tokens_html;
            message = (blockName !== "practice") ? message.replace("{strk-feedback}", `+${nTokens} Tokens`) : message.replace("{strk-feedback}", `Final Streak: ${finalStreak}`); 
            ...
            let feedbackType = lossArray.pop();
            let nTokens = (feedbackType == "oddball" && blockName !== "practice") ? oddBallValue_geom : 0;
            message = (feedbackType == "oddball" && blockName !== "practice") ? oddball_html : noTokens_html;
            message = (blockName !== "practice") ? message.replace("{strk-feedback}", `+${nTokens} Tokens`) : message.replace("{strk-feedback}", `Final Streak: 0`); 
  ...
  */
  ...
  obj.tileGame_howToEarn = function(gameType, gameName_1, gameName_2, pM, color, hex, roundLength, round, nOddBall) {

      let html;

      let pOddBall1 = nOddBall[0] * 10;
      let pOddBall2 = nOddBall[1] * 10;
      let oddBallValue_bern = Math.round(10 * pM);
      let oddBallValue_geom = Math.round(10 * (pM / (1 - pM)));
        ...
        if (round == 1) {
          ...
          html = [`<div class='parent'>
                  ...
                  `<div class='parent' style='height: 550px'>
                    <p>Suppose you achieve a winning streak of 3.<br>If you get a <span style="color:${hex}; font-weight:bold">standard outcome</span>, you'll see this message indicating that you earned 30 tokens.</p>    
                    <div class="header-win" style="top:20%; color:${hex}">Final Streak: 3</div>                           
                    <div class="token-text-win" style="color:${hex}">+30 Tokens</div>
                  </div>`,

                  `<div class='parent' style='height: 550px'>
                    <p>If you achieve a winning streak of 3 and get an <span style="color:red; font-weight:bold">oddball outcome</span>,<br>you'll see this message indicating that you earned ${oddBallValue_geom} tokens.</p>    
                    <div class="header-oddball" style="top:20%">Final Streak: 3</div>                           
                    <div class="oddball-text">+${oddBallValue_geom} Tokens</div>
                  </div>`,

                  `<div class='parent' style='height: 550px'>
                    <p>Suppose you miss the tile after failing to start a streak.<br>If you get a <span style="color:${hex}; font-weight:bold">standard outcome</span>, you'll see this message indicating that you earned 0 tokens.</p>
                    <div class="header-lose" style="top:20%">Final Streak: 0</div>
                    <div class="token-text-lose">+0 Tokens</div>
                  </div>`,

                  `<div class='parent' style='height: 550px'>
                    <p>If you fail to start a streak and get an <span style="color:red; font-weight:bold">oddball outcome</span>,<br>you'll see this message indicating that you earned ${oddBallValue_geom} tokens.</p>
                    <div class="header-oddball" style="top:20%">Final Streak: 0</div>                           
                    <div class="oddball-text">+${oddBallValue_geom} Tokens</div>
                  </div>`,

                  `<div class='parent'>
                    <p>After each streak in the ${gameName_1}, you have a <span style="color:${hex}; font-weight:bold">${100-pOddBall1}%</span> chance of a <span style="color:${hex}; font-weight:bold">standard outcome</span><br>and a <span style="color:red; font-weight:bold">${pOddBall1}%</span> chance of an <span style="color:red; font-weight:bold">oddball outcome</span></p>
                  </div>`,

                  `<div class='parent'>
                    <p>You'll have <b>${speed} time</b> to activate the tile ${asIn} the practice session.</p>
      ...
      if (gameType == 'bern') {
        ...
        if (round == 1) {
          html = [`<div class='parent'>
                  ...
                  `<div class='parent'>
                    <p>In the ${gameName_1}, you'll earn tokens for <b>achieving successes</b>.</p>
                  </div>`,
                 ...
                 `<div class='parent'>
                      <p>You'll have <b>${speed} time</b> to activate the tile compared to the practice session.</p>
                      <p>Accordingly, ${fasterOrSlower} during practice.</p>
                  </div>`];
        } else if (round == 2) {
          html = [`<div class='parent'>
